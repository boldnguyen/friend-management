...
func (r *userR) GetUserID1FriendConnections() FriendConnectionSlice {
	...
	return r.UserID1FriendConnections
}

func (r *userR) GetUserID2FriendConnections() FriendConnectionSlice {
	if r == nil {
		return nil
	}
	return r.UserID2FriendConnections
}
...
func (o *User) UserID1FriendConnections(mods ...qm.QueryMod) friendConnectionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"friend_connections\".\"user_id1\"=?", o.ID),
	)

	return FriendConnections(queryMods...)
}

// UserID2FriendConnections retrieves all the friend_connection's FriendConnections with an executor via user_id2 column.
func (o *User) UserID2FriendConnections(mods ...qm.QueryMod) friendConnectionQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"friend_connections\".\"user_id2\"=?", o.ID),
	)

	return FriendConnections(queryMods...)
}

// RequestorSubscriptions retrieves all the subscription's Subscriptions with an executor via requestor column.
func (o *User) RequestorSubscriptions(mods ...qm.QueryMod) subscriptionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"subscriptions\".\"requestor\"=?", o.Email),
	)

	return Subscriptions(queryMods...)
}

// TargetSubscriptions retrieves all the subscription's Subscriptions with an executor via target column.
func (o *User) TargetSubscriptions(mods ...qm.QueryMod) subscriptionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"subscriptions\".\"target\"=?", o.Email),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserID1FriendConnections(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`friend_connections`),
		qm.WhereIn(`friend_connections.user_id1 in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load friend_connections")
	}

	var resultSlice []*FriendConnection
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice friend_connections")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on friend_connections")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for friend_connections")
	}

	if singular {
		object.R.UserID1FriendConnections = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &friendConnectionR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID1 {
				local.R.UserID1FriendConnections = append(local.R.UserID1FriendConnections, foreign)
				if foreign.R == nil {
					foreign.R = &friendConnectionR{}
				}
...
// LoadUserID2FriendConnections allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserID2FriendConnections(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`friend_connections`),
		qm.WhereIn(`friend_connections.user_id2 in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.UserID2FriendConnections = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &friendConnectionR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID2 {
				local.R.UserID2FriendConnections = append(local.R.UserID2FriendConnections, foreign)
				if foreign.R == nil {
					foreign.R = &friendConnectionR{}
				}
...
func (userL) LoadRequestorSubscriptions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`subscriptions.requestor in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.Email == foreign.Requestor {
...
func (userL) LoadTargetSubscriptions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`subscriptions`),
		qm.WhereIn(`subscriptions.target in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddUserID1FriendConnections adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserID1FriendConnections.
// Sets related.R.UserID1User appropriately.
func (o *User) AddUserID1FriendConnections(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*FriendConnection) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID1 = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			UserID1FriendConnections: related,
		}
	} else {
		o.R.UserID1FriendConnections = append(o.R.UserID1FriendConnections, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &friendConnectionR{
				UserID1User: o,
			}
		} else {
			rel.R.UserID1User = o
...
// AddUserID2FriendConnections adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserID2FriendConnections.
// Sets related.R.UserID2User appropriately.
func (o *User) AddUserID2FriendConnections(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*FriendConnection) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID2 = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			UserID2FriendConnections: related,
		}
	} else {
		o.R.UserID2FriendConnections = append(o.R.UserID2FriendConnections, related...)
	}
